...
##source(paste0(mainDir1,"/1-loaddata.R"))
...
castdata.goingback.ctr2 <- dcast(castdata.goingback.ctr, Country ~ Intention,  value.var= "Frequency")
...
## Comparison of intention - per year of arrival

```{r , echo=FALSE, warning=FALSE, cache=FALSE, fig.width=6, fig.height=7}

castdata.goingback.arrival <- as.data.frame(prop.table(table(data$group_intro.goingback,data$YearArrivalCategory),2))

# prop.table(table(data$YearArrivalCategory, useNA = "ifany"))

names(castdata.goingback.arrival)[1] <- "Intention"
names(castdata.goingback.arrival)[2] <- "Arrival"
castdata.goingback.arrival$Frequency <- paste(round(castdata.goingback.arrival$Freq * 100 ,digits=1),"%",sep="")
castdata.goingback.arrival <- castdata.goingback.arrival[ , c("Arrival","Intention","Frequency")]
...
#castdata.goingback.ctr2 <- dcast(castdata.goingback.ctr, Country + Intention ~ Frequency)


castdata.goingback.arrival2 <- dcast(castdata.goingback.arrival, Arrival ~ Intention,  value.var= "Frequency")

#print(pandoc.table(castdata.goingback.arrival, rownames = NULL, split.table = 300, split.cells = c( "100%"), use.hyphening = TRUE, justify = 'left'))
kable(castdata.goingback.arrival2)


goingback.likert.arrival <- likert(data.goingback, grouping = data$YearArrivalCategory)
# str(goingback.likert.arrival)
plot(goingback.likert.arrival)
...
## Comparison of factors - per year of arrival

The following graph is for those unwilling to return. Respondents could select multiple answers.

```{r , echo=FALSE, warning=FALSE, fig.height=7}

data.reasonsnot.arrival <- as.data.frame(data[ , c( "reasonnot.propertydoc", "reasonnot.well_settled", 
                                            "reasonnot.displacedcommunity", "reasonnot.fearsretaliation", 
                                            "reasonnot.resettlement", "reasonnot.no_educ", 
                                            "reasonnot.fears", "reasonnot.resttleinterest", 
                                            "reasonnot.nosaving", "reasonnot.familydecision", 
                                            ...
                                            "reasonnot.noelect",
                                            "YearArrivalCategory")])
data.reasonsnot.arrival$id <- rownames(data.reasonsnot.arrival)
meltdata.reasonsnot.arrival <- melt(data.reasonsnot.arrival,id=c("id","YearArrivalCategory"))
meltdata.reasonsnot.arrival <- meltdata.reasonsnot.arrival[meltdata.reasonsnot.arrival$value!="Not selected", ]
meltdata.reasonsnot.arrival <- meltdata.reasonsnot.arrival[meltdata.reasonsnot.arrival$value!="Not replied", ]

castdata.reasonsnot.arrival <- as.data.frame(prop.table(table(meltdata.reasonsnot.arrival$value, meltdata.reasonsnot.arrival$YearArrivalCategory),2))
castdata.reasonsnot.arrival$Var1 <-factor(castdata.reasonsnot.arrival$Var1, levels=castdata.reasonsnot[order(castdata.reasonsnot$Freq), "Var1"])



goingback.reasonsnot.arrival  <-  ggplot(castdata.reasonsnot.arrival, aes(x = Var1, y=Freq)) +
  geom_bar( stat = "identity",
            fill="skyblue3",colour="#2a87c8") +
  coord_flip() +
  facet_wrap(~ Var2, ncol=3) + 
...
```{r reasonsctr, echo=FALSE, warning=FALSE, fig.width=7, fig.height=9}
...
```{r reasonsgov, echo=FALSE, warning=FALSE, fig.width=7, fig.height=9}
...
## Ranking of priorities - per per year of arrival

The following graph is for those willing to return.

```{r , echo=FALSE, warning=FALSE, fig.width=7, fig.height=6}

data.priority.arrival <- as.data.frame(data[ !(is.na(data$grouppriority.first)) , c( "grouppriority.first",
                                                                                 "grouppriority.second",
                                                                                 "grouppriority.third",
                                                                                 "YearArrivalCategory")])
data.priority.arrival$id <- rownames(data.priority.arrival)

meltdata.priority.arrival <- melt(data.priority.arrival,id=c("id","YearArrivalCategory"))
castdata.priority.arrival <- as.data.frame(prop.table(table(meltdata.priority.arrival$value, meltdata.priority.arrival$variable,meltdata.priority.arrival$YearArrivalCategory),3))
castdata.priority.arrival$Var1 <-factor(castdata.priority.arrival$Var1, levels=castdata.priority[order(castdata.priority1$Freq), "Var1"])

## Recoding and ordering priorities
castdata.priority.arrival$Var2 <- as.character(castdata.priority.arrival$Var2)
castdata.priority.arrival$Var2[castdata.priority.arrival$Var2=="grouppriority.third"] <- "Third priority"
castdata.priority.arrival$Var2[castdata.priority.arrival$Var2=="grouppriority.second"] <- "Second priority"
castdata.priority.arrival$Var2[castdata.priority.arrival$Var2=="grouppriority.first"] <- "First priority"

castdata.priority.arrival$Var2 <-factor(castdata.priority.arrival$Var2, levels=c("Third priority","Second priority","First priority"))


goingback.priority.arrival  <-  ggplot(castdata.priority.arrival, aes(x = Var1, fill=Var2, y=Freq)) +
  geom_bar( stat = "identity" , colour="#2a87c8") +
  coord_flip() +
  facet_wrap(~ Var3, ncol=3) + 
  guides(fill = guide_legend(reverse=TRUE)) +
