...
### Read the data by Country IRQ.CSV, ARE.CSV, JOR.CSV, change the file name below to run for each country
#rowdata<-read.csv(paste0("./data/","IRQ",".csv" ), header = TRUE)
#data <- subset(rowdata,COO_L1=='Aleppo')


#Modified by Wadih Al-Eid and Awat Salih 10-Apr-2017
...
rowdata<-read.csv(paste0("./data/",CoA,".csv" ), header = TRUE)

### Loopover al COO L1 in the main file

for (CoOL1 in c('Aleppo' , 'Damascus' , 'Hassakeh','Other','Homs',	'Rural Damascus','Idleb','Dara'))
  
{

  data <- subset(rowdata,COO_L1==CoOL1)
...
N_minus1 <- N - 1
Conf_Int <- 5
Conf_Int_sqr <-  Conf_Int ^ 2
Temp1 = Conf_Int_sqr * N_minus1
p_new = 30
q_new = 100 - p_new
dummy = 3.84

n0_new = N / (1 + (Temp1/(p_new*q_new*dummy)))


#n is the sample size based on population provide from the row data
n <- round(n0_new , digits = 0)
...
strFileFullName=paste0("./out/",CoA, CoOL1,".csv")

if (file.exists(strFileFullName))
{
  file.remove(strFileFullName)
}
#write.csv(data_sampled, paste0("./out/",CoA, CoOL1,".csv")) 
...
write.csv(data_sampled2, paste0(strFileFullName) )
