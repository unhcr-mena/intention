...
names(estimation3.coo)[2] <- "# of cases with intention to return"
names(estimation3.coo)[3] <- "Corresponding # of individuals with intention to return"
...
names(estimation4.ctr)[2] <- "# of cases with intention to return"
names(estimation4.ctr)[3] <- "Corresponding # of individuals with intention to return"
...
Based on the phone survey on the intention and perception of Syrian refugees return to Syria performed in April 2017 in Egypt, Iraq and Jordan, it is possible to extrapolate results in order to assess potential numbers of return to Syria for the entire registered population. The analytical approach is based on the following steps:

1.	The relation between the expressed intention to return (i.e. yes or undecided) and characteristics of each respondent (i.e. case) allows to understand the variables that, in terms of probability, are strongly dependent, moderately dependent and independent can influence similar intention to return: 
    *	Strongly dependent variables to the intention to return are: __Country of Asylum__, __Area of Origin__, __Ethnicity__ and __Occupation__;
    *	Moderately dependent variables to the intention to return are:  __Case Size__ & __Year of Arrival__
    *	Independent variables to the intention to return are: __Marital Status of the household representative__, __Female ratio__, __Age Dependency__, __Average age within household__ and __Occurrence of specific Needs__.

2.	To refine the accuracy of the projections with regard to potential of returns and limit sampling bias,  the same three key variables that were used in the original strategy, were applied to define the relative percentage of population segments (__Country of Asylum__, __Area of Origin__ and __Case Size__) per intention (i.e. yes or undecided). Case Size is an important variable to consider in order to calculate projected number of individuals. This percentage can be then applied to the population numbers within each population segment to get the researched estimation. This represents an estimation of __`r  sumyes `__ cases who potentially intends to return and another __`r  sumundecided `__ cases who are potentially undecided to return.
...
```{r , echo=FALSE,  warning=FALSE, message=FALSE, comment = "", tidy = TRUE, fig.height=4, size="small"}
#cat()
kable(estimation4.ctr, caption="__Table__: Estimation of individuals per Country of Asylum with projected intention to return") %>%
kable_styling ( position = "center") %>%
add_footnote ( c("Figures presented here are statistical estimation. Figures are subject to a certain level of confidence and directly influenced by the quality of the original survey dataset."), notation = "number")%>%
...
#pandoc.table(estimation3.coo, caption="__Table__: Estimation of individuals per Country of Asylum", rownames = NULL, split.table = 300, split.cells = c("100%"),   use.hyphening = TRUE, justify = 'left')
#print(pandoc.table(estimation3.coo, caption="__Table__: Estimation of individuals per Country of Asylum", style = "rmarkdown", split.table = 300,  justify = 'left'))
```



3.	The analysis of projected intention to return has been then compared with the recorded spontaneous departure in 2016 & 2017 for Syrian Refugees in Egypt, Iraq & Lebanon for the same population segment. The observed proportion for the same population segments than described above are the following.

```{r , echo=FALSE,  warning=FALSE, message=FALSE, cache=FALSE,comment = "", tidy = TRUE, fig.height=4, size="small"}
kable(country.coa1, caption="__Table__: Comparison between predicted and observed departure per country") %>%
kable_styling ( position = "center") %>%
add_footnote ( c("Figures presented here are statistical estimation. Figures are subject to a certain level of confidence and directly influenced by the quality of the original survey dataset."), notation = "number")%>%
column_spec (3, bold = T)
```
...
4.	The same analysis of variable correlations is confirming the strong dependence to the same variables than the variables dependent with intention to return. The significative differences between observed departure and intention to return lead to the following recommendations: 
    ...
    * Based on differences between observation, intention and realization, it should be possible to adjust a better predictive model to anticipate return to Syria. 

<br>
The main __Key findings__ from the reading of correlation graphs are: 

  * Some refugees staying in Egypt tends to be more inclined to return that those staying in Jordan or Iraq. 
  
  * Some refugees staying in Iraq tends to be more inclined to state that they will __never__ return that those staying in Jordan or Egypt. This is actually a difference with the observed departure where Iraq is the place where correlation with spontaneous departure and country of asylum is the strongest. 
  
  * Refugees coming from Rural Damascus are more inclined to return that those coming from other area of Syria. Refugees coming from Dara and Aleppo as on the other side less inclined to return that those coming from other areas. Refugees coming from Dara are also less inclined to state that they will never return. 
  
  * Kurds refugees are more inclined to state that they will never return that refugees from other ethnicities. 
  
  * Refugees who worked in the agricultural and craft sector (according to [ILO Job classification](http://www.ilo.org/wcmsp5/groups/public/---dgreports/---dcomm/---publ/documents/publication/wcms_172572.pdf)) and in a slightly less significant way, refugees who had qualified work (Manager, Professional, Technicians..) are more inclined to return than other refugees in regards of occupation. 
  
  * Single Refugees are less inclined to return while Refugees registered as couple are more inclined to return. On the other hand, Refugees that registered as large case (more than 5 persons) tends to be more undecided than the others. This pattern is actually confirmed by the correlation with spontaneous departure. 
  
  * In terms of year of arrival, Refugees that arrived in 2013 are more inclined to return while refugees that arrived in 2011 or before as well as refugee who arrived in 2014 are actually less inclined to declare an intention to return. This pattern is confirmed by the correlation with spontaneous departure.



Some __Key questions__ from reading of tables and correlation graphs: 

  * Why do we have such a gap in Egypt between declared intention to return and observed spontaneous departure: Is it linked to the way spontaneous departure are recorded or to a new specific event that happened in the past months, or the way questions were raised during the survey? 
  * Why do we observed so many recorded spontaneous departure, while the declared intention to return is comparatively lower than the other countries? Can this be interpreted as an hypothesis that all Kurds that had to return already did it now?
