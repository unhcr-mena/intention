...
author: "Prepared by UNHCR MENA Protection Services - Draft Document"
...
```{r setup, include=FALSE, echo=FALSE, warning=FALSE, message=FALSE, cache=FALSE}
...
names(frame1)[3] <- "Decied to return within the next year"
############################################################################
...
names(estimation3.coo)[3] <- "# of individuals with intention to return"
names(estimation3.coo)[4] <- "Average case size"
...
estimation4.ctr <-aggregate(. ~ ctr, estimation4.less, sum)
...
names(estimation4.ctr)[3] <- "# of individuals with intention to return"
names(estimation4.ctr)[4] <- "Average case size"
...
sumundecided <- round(sum(estimation5.ctr$numUndecided),digits=2)
sumyes <- round(sum(estimation5.ctr$numyes),digits=2)
...
names(progres.count4)[1] <- " Asylum & Area of Origin"
...
Based on the intention survey performed in Egypt, Iraq and Jordan, it is possible to extrapolate results in order to assess potential numbers of return to Syria. The analytical approach is based on the following steps:

1.	The relation between intention to return (yes or undecided) and different variables that can describe the characteristics of case allow to understand the variables that, in terms of probability, are strongly dependent, moderately dependent and independent:  
    *	The variable strongly dependent to the intention to return are __Country of Asylum__, __Area of Origin__, __Ethnicity__ and __Occupation__;
    *	The variables moderately dependent to the intention to return are __Case Size__, & 
    *	The variables independent to the intention to return are __Marital Status of the household representative__, __Female ratio__, __Age Dependency__, __Average age within household__ and __Occurrence of specific Needs__.
 
<br >   

2.	To refine the accuracy of the projections and limit sampling biais, the __Country of Asylum__, __Area of Origin__ and __Case Size__ variables were then used to define the relative percentage of population segment (Country of Asylum, Area of Origin and Case Size) per intention (yes or undecided) (i.e. post-stratify the sample by computing weight for each record within the sample). Case Size is an important variable to consider in order to calculate projected number of individuals. This percentage can be then applied to the population numbers within each population segment to get the researched estimation. This represents an estimation of __`r  sumyes `__ cases who potentially intends to return and another __`r  sumundecided `__ caes who are potentially undecided to return.
...
add_footnote ( c("Figures presented here are estimation"), notation = "number")%>%
column_spec (3, bold = T)


#print(pandoc.table(estimation4.ctr, caption="__Table__:Estimation of individuals per Country of Asylum", rownames = NULL, split.table = 300, split.cells = c("100%"),   use.hyphening = TRUE, justify = 'left'))

#pandoc.table(estimation4.ctr, caption="__Table__:Estimation of individuals per Country of Asylum", style = "rmarkdown", split.table = 300,  justify = 'left')
...
4.	The same analysis of variable correlations is confirming the strong dependence with  __Country of Asylum __,  __Area of Origin __ and  __Case Size __.
...
This report is the second part of the exploration of the intention survey. The objective is to analyze correlations between variables, to build projection and to compare declared intention with observed spontaneous departure.



# Analysis of Correlation

The analysis of correlation is done using a statistical test (called [Chi-squared](https://en.wikipedia.org/wiki/Chi-squared_test)). This test is used to determine if there is a significant relationship between two nominal categorical variables: It is used to evaluate the probability for two variables to be dependent from each other and it allows to visualise the details of the correlation between each modality of the variables.
...
kable(progres.count4, caption="__Table__:Proportion of spontaneous departure") %>%
kable_styling ( position = "center") %>%
add_footnote ( c("Figures presented here are estimation"), notation = "number")%>%
column_spec (3, bold = T)
```
...
progres.reg <- progresall[ , c("return", "CountryAsylum" , "Case.size" ,  "YearArrivalCategory",
 "female.ratio" , "dependency" , "dem_ethnCat", "occupationcat", "cool1Cat1" )]
